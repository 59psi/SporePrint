"""Anden networked dehumidifier driver — same shape as Quest."""
...
class AndenDriver(HttpVendorDriver):
    name: ClassVar[str] = "anden"
    tier_required: ClassVar[str] = "free"
    config_schema: ClassVar[type[BaseModel]] = AndenConfig

    async def test_connection(self) -> IntegrationHealth:
        cfg: AndenConfig | None = self._cfg  # type: ignore[assignment]
        ...
        return IntegrationHealth(state="ok", details={"fields": list(data.keys())})
    ...
    async def poll_once(self) -> tuple[int, dict[str, Any]]:
        ...
        data = await self._fetch_status(cfg)
        node_id = f"anden:{urlparse(cfg.base_url).netloc}"
        rows = 0
        now = time.time()
        for k, sp in [
            ("temp_c", "temp_c"),
    ...
    async def _fetch_status(self, cfg: AndenConfig) -> dict[str, Any]:
        ...
        if resp.status_code >= 400:
            raise RuntimeError(f"Anden: HTTP {resp.status_code}: {resp.text[:200]!r}")
        try:
            body = resp.json()
        except ValueError as exc:
            raise RuntimeError(f"Anden: non-JSON response: {exc}") from exc
